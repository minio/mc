...
// copyArgs2URLs extracts source and target URLs from command-line args.
func copyArgs2URLs(args cli.Args) (sourceURLs []string, targetURL string) {
	if !isMcConfigExist() {
...
func runCopyCmd(ctx *cli.Context) {
	...
	// extract source and target URLs.
	sourceURLs, targetURL := copyArgs2URLs(ctx.Args())
