...
var (
	eventsAddFlags = []cli.Flag{
		cli.StringFlag{
			Name:  "events",
			Value: "all",
			Usage: "Filter specific type of events. Could be `put`, `remove` or `all`.",
		},
		cli.StringFlag{
			Name:  "prefix",
...
var eventsAddCmd = cli.Command{
	Name:   "add",
	Usage:  "Add new bucket notification",
...
EXAMPLES:
   ...
   2. Enable bucket notification with filters parameters
     $ mc events {{.Name}} s3/mybucket arn:aws:sqs:us-west-2:444455556666:your-queue --events put --prefix photos/ --suffix .jpg
`,
}
