...
func migrateSessionV6ToV7() {
	for _, sid := range getSessionIDs() {
		sessionV7, err := loadSessionV7(sid)
		...
		if sessionV7.Header.Version == "7" { // It is new format.
			return
		}
		/*** Remove all session files older than v7 ***/
		...
		console.Println("Removing unsupported session file ‘" + sessionFile + "’ version ‘" + sessionV7.Header.Version + "’.")
		if e := os.Remove(sessionFile); e != nil {
			fatalIf(probe.NewError(e), "Unable to remove version ‘"+sessionV7.Header.Version+"’ session file ‘"+sessionFile+"’.")
		}
		if e := os.Remove(sessionDataFile); e != nil {
			fatalIf(probe.NewError(e), "Unable to remove version ‘"+sessionV7.Header.Version+"’ session data file ‘"+sessionDataFile+"’.")
		}
	}
}
...
func (s *sessionV7) setGlobals() {
	s.Header.GlobalBoolFlags["no-progressbar"] = globalNoBar
...
func (s sessionV7) restoreGlobals() {
	noBar := s.Header.GlobalBoolFlags["no-progressbar"]
