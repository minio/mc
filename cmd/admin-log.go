...
var adminLogCommand = cli.Command{
	Name:            "log",
	Usage:           "show console logs for MinIO server",
	Action:          mainAdminLog,
	OnUsageError:    onUsageError,
	Before:          setGlobalsFromContext,
	Flags:           append(adminLogFlags, globalFlags...),
...
func checkAdminLogSyntax(ctx *cli.Context) {
	if len(ctx.Args()) == 0 || len(ctx.Args()) > 3 {
		cli.ShowCommandHelpAndExit(ctx, "log", 1) // last argument is exit code
	}
}
...
// mainAdminLog - the entry function of log command
func mainAdminLog(ctx *cli.Context) error {
