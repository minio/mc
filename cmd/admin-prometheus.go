...
var prometheusConfigFlags = []cli.Flag{
	cli.BoolFlag{
		Name:  "generate",
		Usage: "generate a prometheus config",
	},
}

var adminPrometheusCmd = cli.Command{
	Name:            "prometheus",
	Usage:           "provides prometheus config",
	Action:          mainAdminPrometheus,
	Before:          setGlobalsFromContext,
	Flags:           append(prometheusConfigFlags, globalFlags...),
...
func checkAdminPrometheusSyntax(ctx *cli.Context) {
	if len(ctx.Args()) != 1 {
		cli.ShowCommandHelpAndExit(ctx, "prometheus", 1) // last argument is exit code
	}
}
...
func mainAdminPrometheus(ctx *cli.Context) error {
	...
	isGenerate := ctx.Bool("generate")
	if !isGenerate {
		cli.ShowCommandHelpAndExit(ctx, "prometheus", 1)
	}
